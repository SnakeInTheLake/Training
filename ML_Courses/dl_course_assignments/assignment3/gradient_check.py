...
def check_gradient(f, x, delta=1e-5, tol=1e-4):
    ...
    while not it.finished:
        ...
        temp_plus[ix] += delta
        temp_minus[ix] -= delta
        numeric_grad_at_ix = (f(temp_plus)[0] - f(temp_minus)[0]) / (2 * delta)
...
def check_model_gradient(model, X, y,
    ...
    """
    params = model.params()
    from layers import softmax, cross_entropy_loss
    for param_key in params:
        print("Checking gradient for %s" % param_key)
        param = params[param_key]
        initial_w = param.value
        # print(f'param: {param_key}, x: {initial_w}')
        # print(f'probs: {softmax(initial_w)}')
